...
func (d *Device) GetFileHandle(storageID, root uint32, filePath string) (uint32, error) {
	...
	for k, partName := range parts {
		...
		// 0xFFFFFFFF ??
		if err := d.GetObjectHandles(storageID, 0x0, currentDir, &handles); err != nil {
			return 0, err
		}

		for _, handle := range handles.Values {
			obj := ObjectInfo{}
			if err := d.GetObjectInfo(handle, &obj); err != nil {
				// TODO: return instead?
			...
			isDir := obj.ObjectFormat == OFC_Association
			if lastPart {
				if isDir {
					return 0, fmt.Errorf("found %v, but it is a directory", partName)
				}
				// found it
				println("FOUND HANDLE FOR", partName)
				return handle, nil
			}

			if !isDir {
				return 0, fmt.Errorf("found %v part, but it is a file, not a directory", partName)
			}
